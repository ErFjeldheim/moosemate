...
The issues on git.ntnu.no have been described with checkboxes to meet the requirements, and when these were all checked, an PR was made. A peer, or more, in the group were put as reviewers, and tried running the branch locally. The reviewer also reviewew the code itself, to look for bugs, duplications etc. This is sometimes hard to do, as the amount of code can feel overwhelming. However, as we adapted to pair programming for larger features, there were rarely any big bugs to be discovered, as the code was already peer-reviewed in a type of way. 
